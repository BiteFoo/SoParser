...
#         print 'call EncrySection section Name  %s' %sname
#         num = self.elf32_ehdr.e_shnum
#         name_section_offset = self.section_header_table[self.elf32_ehdr.e_shstrndx].sh_offset        
#         self.so.seek(name_section_offset)
#         l = self.section_header_table[self.elf32_ehdr.e_shstrndx].sh_size
...
#             name = []
#             while True:
#                 #print 'table name ', self.section_name_table[idx]
...
#             new_data.append(chr((ord(i)^11)))#将每个字符 ^ 11
...
class SO:
    ...
    def EncrySection(self,sname):
        ...
        for i in xrange(num):
            ...
            #找到特定的section
            if "".join(name) == sname:
                print '********* find it section name="%s"' %sname , 'index=%d'%i ,' idx =%d'%idx
                break
...
def clear_vars():
    ...
    current = sys.path[0]
    for path in dirs:
        tmp = current +os.sep+path
        if os.path.isdir(tmp):
            print 'clear locals %s'%(tmp)
            shutil.rmtree(tmp)

def iterator_file():
    current = sys.path[0]
    file_list =[]
    sopath = current+os.sep+"libs"
    for root,subs,files in os.walk(sopath):
        for file in files:
...
def start():
    ...
    files = iterator_file()
    if files == None:
        print 'file list is None ,please check it ..\n'
        sys.exit(0)
    for file in files:
        encryption(file)
